...
const SIMPLE_COMP_FILEPATH: &str = "./build/BnanR-Sample-Shaders/simple-raytrace.comp.spv";
...
impl SimpleSystem {
    ...
    pub fn draw(&self, frame_info: &BnanFrameInfo) {
        ...
        let blit_info = vk::BlitImageInfo2::default()
            .dst_image(frame_info.swapchain_image.lock().unwrap().image)
    ...
    fn create_storage_buffers(device: ArcMut<BnanDevice>, count: u32) -> Result<[BnanBuffer; FRAMES_IN_FLIGHT]> {
        ...
        let images: [BnanBuffer; FRAMES_IN_FLIGHT] = vec?.try_into().map_err(|_| anyhow!("something went wrong while creating storage buffers"))?;
        Ok(images)
    }
